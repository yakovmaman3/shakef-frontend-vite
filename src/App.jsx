...
function App() {
  ...
  return (
    <div className="min-h-screen">
      <HomePage setActiveSection={setActiveSection} setShowPhone={setShowPhone} />
      {(activeSection || showPhone) && (
        <div
          className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm z-40 flex items-center justify-center transition-all duration-500"
          onClick={handleOverlayClose}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-white/10 text-white p-8 rounded-xl max-w-xl w-full backdrop-blur-md shadow-xl transition-all scale-100"
          >
            {activeSection === "about" && (
              <div>
            ...
            {showPhone && (
              <div className="text-center space-y-4">
                ...
                <p className="text-lg">📞 055-2270388</p>
              </div>
            )}
          </div>
        </div>
